...
class CommonApi extends REST_Controller
{
    ...
    public function social_signup_put()
    {
        try {
            ...
            if (empty($validation) && in_array($params['user_type'], ['patients', 'providers'])) {
                ...
                /** ******* CEHCK EMAIL ************* */
                $ext_name=($params['user_type']=='patient')?QUICKBLOX_PATIENT_EMAIL_EXTENSION : QUICKBLOX_PROVIDER_EMAIL_EXTENSION;
                if (empty($checkSocial) && $params['social_token_id'] != '' && $params['social_type'] != '') {
                    ...
                    if(isset($params['email']) && !empty($params['email'])){
                        ...
                        if($emailUserExists){
                            ...
                            $insertdata['status'] = $emailUserExists[0]['status'];
                   ...
                   if($checkSocial==1 || $checkSocial==true){
                        $response["status"] = 0;
    ...
    public function otp_sent_post()
    {
        ...
        if (empty($validation) && in_array($params['user_type'], ['patient', 'provider']) && isset($url[1]) && in_array($url[1],['resend','send'])) {
            ...
            $otpUserData = $this->CommonModel->getCollectionData($params['user_type'] . 's', $wheres, ['email', 'mobile','country_code']);
            if ($otpUserData) {
                $valid=0;
                if($params['key_type']=='email'){
                    $digits = 6;
                    $updateValue = rand(pow(10, $digits-1), pow(10, $digits)-1);
                    $message= $updateValue.' is your one time password(OTP) for emial verification.';
                    send_email($otpUserData[0]['email'],'Online Appointment Email Verification',$message);
                    $response["status"] = 1;
                ...
                if($valid){
                    if($url[1]=='resend'){
                        ...
                        /* if(isset($params['userid']) && isset($params['country_code'])){
                            $updatedData=['otp_or_sessionId'=>(string)$updateValue,'country_code'=>$params['country_code'],'mobile'=>$params['key_value']];
                        }else{
                    ...
                    $response["status"] = 0;
                    $response['message'] = 'Please enter valid key type value.';
                }
            } else {
                $response["status"] = 0;
